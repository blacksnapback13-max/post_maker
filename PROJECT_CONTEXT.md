...
Подготовлена рабочая сборка v1.2.5: image prompt очищен от постерных формулировок, Pollinations enhancement выключен по умолчанию, negative prompt запрещает буквы, псевдотекст, логотипы, кресты, печати и watermark внутри AI-фона.

## Следующие шаги

- Выкатить v1.2.5 на сервер и проверить production env-переменные.
- Проверить продовый `/api/config`: должен отдавать `version: "1.2.5"`, `imageProviders` и `imageProviderUsage`.
...
- Текст стиха, место Писания и логотип должны появляться только canvas-оверлеем; AI-фон должен быть wordless/background-only.
