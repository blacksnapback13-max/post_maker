...
Подготовлена рабочая сборка v1.2.6: Pollinations/Diffusion positive prompt стал только визуальным, запреты вынесены в provider-level negative prompt, а canvas дополнительно смягчает псевдобуквы на таких фонах перед наложением стиха.

## Следующие шаги

- Выкатить v1.2.6 на сервер и проверить production env-переменные.
- Проверить продовый `/api/config`: должен отдавать `version: "1.2.6"`, `imageProviders` и `imageProviderUsage`.
