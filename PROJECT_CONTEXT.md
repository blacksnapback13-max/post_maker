...
Подготовлена рабочая сборка v1.2: закреплена free-only AI policy, добавлен changelog, экспортные пресеты соцсетей и локальная история сгенерированных постов/фонов.

## Следующие шаги

- Прогнать v1.2 smoke/UI-проверки и собрать релизный архив.
- Выкатить v1.2 на сервер и проверить production env-переменные.
- Добавить экспорт сразу в несколько форматов.

## Ошибки и риски

- Для AI-фона нужен GEMINI_API_KEY в .env; если image quota недоступна, API возвращает локальный SVG fallback-фон.
