...
const PARENT_ENV_PATH = path.join(APP_ROOT, "..", ".env");
  ...
  return http.createServer(async function (request, response) {
    try {
      ...
      if (request.method === "POST" && requestUrl.pathname === "/api/generate-background") {
        ...
        const image = await requestBackgroundImage(promptInput, prompt, model, referenceImage);
